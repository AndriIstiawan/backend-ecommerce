<script>
    ...
    function refreshNumberFormat() {
        $('.idr-currency').on('keyup', function () {
            var number = $(this).val();
    ...
    $('.brand-unit').select2({
        ...
        var picture = $(this).find('option:selected').attr('picture');
        $('.brand-picture').attr('src', '{{ asset("img/brands") }}/'+picture);
        $('.brand-picture').css('display', 'block');
        $(this).valid();
    });
    ...
    function save(action) {
        if ($('#jxForm').valid() && ( $('#jxForm input[name="image[]"]').length || $('#jxForm input[name="fileImage[]"]').length )) {
            $('.showProgress').click();
            $.ajax({
                xhr: function () {
            ...
            if($('#jxForm input[name="picture[]"]').length == 0 && $('#jxForm input[name="fileImage[]"]').length == 0){
    ...
    function act(action) {
        switch (action) {
            case 'continue':
                ...
                break;
            case 'exit':
                window.open("{{ route('product.index') }}/?edit=product", "_self");
        }
    }

    @if($product->variant)
    $('.price-col').html($('.fade .price-range').html());
    var minPrice = parseFloat('{{$product->price[0]["min"]}}');
    var maxPrice = parseFloat('{{$product->price[0]["max"]}}');
    $('#jxForm input[name="minPrice"]').val(minPrice.toLocaleString('id-ID'));
    $('#jxForm input[name="maxPrice"]').val(maxPrice.toLocaleString('id-ID'));
    @else
    $('.price-col').html($('.fade .price-input').html());
    var prodPrice = parseFloat('{{$product->price[0]["min"]}}');
    $('#jxForm input[name="prodPrice"]').val(prodPrice.toLocaleString('id-ID'));
    @endif
