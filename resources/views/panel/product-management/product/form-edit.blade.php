                        ...
                        <div class="card-body">
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    ...
                                    <?php
                                    $image_url = $prodImage['filename'];
                                    if(strpos($prodImage['filename'], 'https://') === false && strpos($prodImage['filename'], 'http://') === false ){
                                        $image_url = asset('img/products/'.$prodImage['filename']);
                                    }
                                    ?>
                                    <div class="pull-left">
                                        <a class="btn" style="cursor:default;">
                                            <img class="rounded picturePrev" src="{{ $image_url }}" style="width: 150px; height: 150px;">
                                        </a><br>
                            ...
                            <div class="form-group row">
                                <label class="col-md-3 col-form-label" for="brand">*Brand
                                    <br>
                                    <small class="text-muted">Setting brand of product.</small>
                                </label>
                                <div class="col-md-9">
                                    <div class="row">
                                        <div class="col-md-4">
                                            <select name="brand" class="form-control brand-unit" aria-describedby="brand-error">
                                                <option value=""></option>
                                                @foreach($brands as $brand)
                                                <?php
                                                $selected = "";
                                                if(isset($product->brand[0]['_id'])){
                                                    if($product->brand[0]['_id'] == $brand->id){
                                                        $selected = "selected";
                                                    }
                                                }
                                                ?>
                                                <option value="{{$brand->id}}" picture="{{$brand->picture}}" {{$selected}}>{{$brand->name}}</option>
                                                @endforeach
                                            </select>
                                            <em id="brand-error" class="error invalid-feedback"></em>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-4">
                                            <center><img class="brand-picture" src="{{ (isset($product->brand[0]['picture'])?asset('img/brands/'.$product->brand[0]['picture']):'') }}" style="width: 150px; height: 70px; margin-top:5px; {{ (isset($product->brand[0]['picture'])?'':'display:none;') }}"></center>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <tr>
                                                <td class="text-center">
                                                    <a class="btn vars-btn" style="cursor:default;margin:0 auto;padding:0;">
                                                        <?php
                                                        $image_url = ($prodVar['image']!=''||$prodVar['image']!=null?$prodVar['image']:'');
                                                        if(strpos($prodVar['image'], 'https://') === false && strpos($prodVar['image'], 'http://') === false ){
                                                            $image_url = ($prodVar['image']!=''||$prodVar['image']!=null?url('/img/products/'.$prodVar['image']):'');
                                                        }
                                                        ?>
                                                        <img class="rounded picturePrev" src="{{ ($image_url !=''?$image_url:asset('img/add-photo.png')) }}" style="width: 110px; height: 110px;">
                                                    </a>
                                                    <br>
                                                ...
                                                <td style="vertical-align:top;">
                                                    <input type="text" name="varStock[]" class="form-control text-right input-number" placeholder="00" onchange="priceStock('stock')" value="{{$prodVar['stock']}}">
                                                </td>
