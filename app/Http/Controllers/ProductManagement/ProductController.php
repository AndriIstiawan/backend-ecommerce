...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        $brands = Brand::all();
        return view('panel.product-management.product.form-create', ['categories' => $categories, 'brands' => $brands]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $product->brand = [Brand::find($request->brand)->toArray()];
        ...
        if ($request->variants && count($request->variants) > 0) {
            ...
            for ($i = 0; $i < count($request->variants); $i++) {
                ...
                $arrVar[] = [
                    ...
                    'stock' => (double) $request->varStock[$i],
                ];
            }
    ...
    {
        $products = product::select('_id', 'name', 'sku', 'price', 'stock', 'categories', 'variant', 'created_at');
        // //offset
        // $products = $products->skip((integer)$request->start);
        // //limit
        // $products = $products->take((integer)$request->length);
        ...
        return Datatables::of($products)
            ...
            ->addColumn('product_id', function ($products) {
                 return $products->id;
             })
            ->addColumn('categories_name', function ($products) {
                if ($products->categories) {
                    return implode(', ', array_column($products->categories, 'name'));
                } else {
                    return "0";
                }
            })
            ...
            ->rawColumns(['priceCol', 'product_id', 'variants', 'action'])
            ->make(true);
    }
    ...
    {
        switch ($action) {
            case "list-data":return $this->listData($request);
    ...
    public function edit($id)
    {
        ...
        $categories = $categories->toArray();
        $brands = Brand::all();
        return view('panel.product-management.product.form-edit')->with(['product' => $product, 'categories' => $categories, 'brands' => $brands]);
    }
